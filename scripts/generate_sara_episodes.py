"""
scripts/generate_sara_episodes.py
----------------------------------
Offline SARA episode generator — § 7703 (married/unmarried tax status).
...
Usage:
    python scripts/generate_sara_episodes.py \\
        --out envs/syllogym_env/server/data/sara_episodes.json \\
        --count 500 \\
        --seed 42
"""
...
_Q_FOLLOWUP = [
    ...
    "With this new information, does the taxpayer's § 7703 status change?",
    "After this update, is the taxpayer still treated as unmarried under § 7703?",
    "Does this new fact affect the individual's unmarried status under § 7703?",
    "Given this clarification, is the person still treated as unmarried?",
    "How does this new development affect the § 7703 analysis?",
...
def generate_episodes(rng: random.Random, count: int) -> list[Episode]:
    episodes = []
    for _ in range(count):
        n = rng.randint(_MIN_TURNS, _MAX_TURNS)
        episodes.append(_generate_episode(rng, n))
    return episodes
...
def main() -> None:
    ...
    p.add_argument("--out", default="envs/syllogym_env/server/data/sara_episodes.json")
    ...
    print(f"Generating {args.count} § 7703 episodes (seed={args.seed})...")
    episodes = generate_episodes(rng, args.count)

    # Stats
    twist_count = sum(1 for ep in episodes for t in ep.turns if t.is_twist)
    turn_dist: dict[str, int] = {}
    for ep in episodes:
        turn_dist[ep.task_name] = turn_dist.get(ep.task_name, 0) + 1
    yes = sum(1 for ep in episodes for t in ep.turns if t.correct_answer == "Yes")
    no = sum(1 for ep in episodes for t in ep.turns if t.correct_answer == "No")
    total = yes + no

    print(f"Episodes: {len(episodes)}, twist turns: {twist_count}")
    print(f"Distribution: {dict(sorted(turn_dist.items()))}")
    print(f"Answer balance: Yes={yes} ({yes/total:.1%}), No={no} ({no/total:.1%})")

    out_path.write_text(json.dumps([episode_to_dict(ep) for ep in episodes], indent=2))
